document.getElementById('password').addEventListener('input', function(e) {
    ...
    const currentMoonPhase = getMoonPhaseEmoji();
    ...
    const rules = [
        ...
        { id: 8, text: 'Your password must include one of our sponsors.', valid: /(?:pepsi|sturbucks|shell)/ },
    ...
    for (let i = 0; i < rules.length; i++) {
        ...
        if (existingHint) {
            const isValid = typeof rule.valid === 'function' ? rule.valid(password) : rule.valid.test(password);
            if (isValid) {
            ...
            // Kural 19 için butonu görünür yap
            if (rule.id === 19 && !document.getElementById('bold-button-container')) {
                createBoldButton();
                isBoldButtonVisible = true;
            }
        }
    }

    // Eğer kural 19 aktif değilse bold butonunu gizle
    if (!isBoldButtonVisible) {
        const boldButtonContainer = document.getElementById('bold-button-container');
        if (boldButtonContainer) {
            boldButtonContainer.style.display = 'none';
        }
    } else {
        const boldButtonContainer = document.getElementById('bold-button-container');
        if (boldButtonContainer) {
            boldButtonContainer.style.display = 'block';
        }
    }

    const hintList = Array.from(hints.children);
    hintList.sort((a, b) => {
        const aValid = a.classList.contains('valid');
        const bValid = b.classList.contains('valid');

        if (aValid && !bValid) return 1;
        if (!aValid && bValid) return -1;

        return parseInt(b.id.replace('hint-', '')) - parseInt(a.id.replace('hint-', ''));
    });
...
function createBoldButton() {
    const boldButtonContainer = document.createElement('div');
    boldButtonContainer.id = 'bold-button-container';
    boldButtonContainer.style.marginTop = '10px';

    const boldButton = document.createElement('button');
    boldButton.textContent = 'Bold Selected Text';
    boldButton.style.padding = '5px 10px';
    ...
    boldButton.addEventListener('click', function() {
        ...
        if (start !== end) {
            const selectedText = passwordInput.value.slice(start, end);
            const boldText = `<b>${selectedText}</b>`;
            const newPassword = passwordInput.value.slice(0, start) + boldText + passwordInput.value.slice(end);
    ...
    document.getElementById('password').parentNode.appendChild(boldButtonContainer);
}
